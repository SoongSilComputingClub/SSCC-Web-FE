...
type LocalGenderData = {
  maleCount: number;
  femaleCount: number;
  malePercentage: number; // 0~100
  femalePercentage: number; // 0~100
};

export function GenderStatsModal({ isOpen, onClose }: GenderStatsModalProps) {
  const [data, setData] = useState<LocalGenderData | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const items = (result.data ?? []) as ApplyFormItem[];

        let male = 0;
        let female = 0;

        for (const it of items) {
          const g = it.gender?.toLowerCase?.() ?? '';
          if (g === 'male') male += 1;
        ...
        const femalePct = total === 0 ? 0 : 100 - malePct; // 합 100 맞추기
